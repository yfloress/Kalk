...
pub fn contextual_field_help(app: &App, m: &Messages) -> String {
    match app.input_field {
        ...
        InputField::WeightedEvals => m.help_weighted_evaluations.to_string(),
...
pub fn draw_category_help_overlay(frame: &mut Frame, m: &Messages) {
    ...
    let help_lines = vec![
        ...
        Line::from(""),
        Line::from(vec![
            Span::styled(format!("{}: ", m.weighted_evaluations), label_style),
            Span::styled(m.help_weighted_evaluations, desc_style),
        ]),
    ];
